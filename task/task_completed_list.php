...
if (isset($uid)) {
    $sql = "select * from task_main where status=3 and uid=13 and DATE_FORMAT(FROM_UNIXTIME(start_time),'%Y%m%d%H%i%s')>=$start_date and $end_date>=DATE_FORMAT(FROM_UNIXTIME(start_time),'%Y%m%d%H%i%s')";
